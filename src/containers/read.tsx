...
// import {StatusBar as StatusBarManager} from 'react-native';
...
const Read: React.FC<Props> = props => {
  ...
  useUpdateEffect(() => {
    ...
    console.log(props.chapterList[book.preChapter].uri,"页面网址");
    setLoading(true);
  }, [book]);
  ...
  useUpdateEffect(() => {
    ...
    console.log(msg,textXY,fontState,"切割完成后数据的视线");
    console.log(splitBook(msg, textXY, fontState),"切割完成后的打印的数据");
    console.log(pageBook,"切割完成后的pageBook");
    setLoading(false);
    setCurPage(0);
  }, [msg, fontState]);
  //设备宽度
  ...
  let statusBarHeight;
  if (Platform.OS === 'ios') {
    // StatusBar.getHeight((height: number) => {
  ...
  const onWebViewLoad = () => {
    console.log("阅读页面执行，并且注入数据");
    const getLinksScript = `
    let content=document.querySelector('#chaptercontent');
...
`;
  ...
  const onMessageReceived = (event: WebViewMessageEvent) => {
    console.log(event.nativeEvent.data,"储存数据");
  ...
  return (
    <View _dark={{background: 'dark.100'}}>
      ...
      <Pressable
        ...
        onPress={e => {
          ...
          if (x < deviceW * 0.3) {
            ...
            //curPage当前页面
            //pageBook.length本章节页面
            //book.preChapter，书的总页面
            console.log("right");
            ...
            console.log("center",curPage,pageBook.length,book.preChapter);
      ...
      <ReadFooter
        ...
        bookMsg={{name: "string",preChapter: 222,pUri: "1111",}}
        isOpen={footerIsOpen}
      />
    </View>
  );
